...
const Head = () => {
  ...
  const userMenu = (
    <Menu>
      <Menu.Item onClick={() => console.log("open prf")}>
        {" "}
        {"Profile"}{" "}
      </Menu.Item>
  ...
  const fetchNotificationList = async (_) => {
    try {
      ...
      resp?.data?.fetchNotificationList
        ? (setNotificationListData(resp.data.fetchNotificationList),
          setShowDropdwn(true))
        : null;
    } catch (error) {
      console.log(error);
          ...
          <Menu.Item key={idx}>
            <div>
              ...
              <div style={{ textAlign: "end", fontSize: 10 }}>
                {moment
                  .unix(
                    notific.created_at.substr(0, notific.created_at.length - 3)
                  )
                  .format("llll")}
              </div>
            </div>
    ...
    <Header className={styles.header}>
      <div className={styles.userPanel}>
        <Dropdown
          ...
          onClick={(_) => fetchNotificationList()}
        >
