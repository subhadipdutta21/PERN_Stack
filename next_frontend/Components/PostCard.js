  ...
  const fetchComments = async (_) => {
    ...
    try {
      let resp = await client.query({
        ...
        variables: { input: { post_id: postID } },
      });
    ...
    <>
      ...
      <Card
        ...
        actions={[
          ...
          likes.length > 0 ? (
            <Badge size="small" count={likes.length}>
              {" "}
              <HeartFilled />{" "}
            </Badge>
          ) : (
          ...
          <div onClick={() => setShow(!show)}>
            {comments.length > 0 ? (
              <Badge size="small" count={comments.length}>
                {" "}
                <MessageOutlined />{" "}
              </Badge>
            ) : (
      ...
      >
        <Skeleton loading={false} avatar active>
          <Meta
            avatar={dp}
            title={name}
            description={<PostBody content={content} />}
          />
        </Skeleton>
      </Card>
